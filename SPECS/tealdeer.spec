...
install -m0755 target/%{_target_cpu}-unknown-linux-musl/release/tldr %{buildroot}/usr/bin/tldr
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/tealdeer.files

%files -f %{_builddir}/tealdeer.files
%changelog
