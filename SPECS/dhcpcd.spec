...
Version:        10.3.2
Release:        1%{?dist}
Summary:        dhcpcd DHCP client (oxide)
License:        BSD-2-Clause
Source0:        dhcpcd-10.3.2.tar.gz

%description
dhcpcd DHCP client (oxide)

%prep
%setup -q -n dhcpcd-10.3.2
...
./configure --build=$(gcc -dumpmachine) --host=$TRIPLE --prefix=/usr --sysconfdir=/etc --dbdir=/var/db/dhcpcd --libexecdir=/usr/lib/dhcpcd --without-udev && make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/dhcpcd.files

%files -f %{_builddir}/dhcpcd.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 10.3.2-1
