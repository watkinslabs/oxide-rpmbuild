...
Version:        1.3.1
Release:        1%{?dist}
Summary:        GNU MPC complex math library (oxide)
License:        LGPL-3.0-or-later
Source0:        mpc-1.3.1.tar.gz

%description
GNU MPC complex math library (oxide)
...
./configure --build=x86_64-pc-linux-gnu --host=$TRIPLE --prefix=/usr --with-gmp=$SYS/usr --with-mpfr=$SYS/usr
make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/mpc.files

%files -f %{_builddir}/mpc.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 1.3.1-1
