...
./configure --build=x86_64-pc-linux-gnu --host=$TRIPLE --cache-file=config.cache --prefix=/usr --disable-md2man --disable-xxhash --disable-zstd --disable-lz4 --disable-openssl --disable-roll-simd --disable-roll-asm --with-included-popt --with-included-zlib
make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/rsync.files

%files -f %{_builddir}/rsync.files
%changelog
