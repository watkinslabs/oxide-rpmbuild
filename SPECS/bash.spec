...
CFLAGS="-Os -D_GNU_SOURCE -std=gnu89 -I$SYS/usr/include -Wno-implicit-function-declaration -Wno-incompatible-pointer-types -Wno-int-conversion $UAPI" \
LDFLAGS="-Wl,-rpath,/usr/lib -Wl,-rpath-link,$SYS/usr/lib -L$SYS/usr/lib " \
PKG_CONFIG_PATH="$SYS/usr/lib/pkgconfig" \
./configure --build=x86_64-pc-linux-gnu --host=%{_target_cpu}-linux-musl --cache-file=config.cache --without-bash-malloc --disable-nls --enable-readline --enable-history --disable-net-redirections --disable-help-builtin --prefix=/usr
make %{?_smp_mflags}
