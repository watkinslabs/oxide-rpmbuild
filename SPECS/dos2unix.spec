...
Version:        7.5.2
Release:        1%{?dist}
Summary:        dos2unix (oxide)
License:        BSD-2-Clause
Source0:        dos2unix-7.5.2.tar.gz
...
make CC="$CC" ENABLE_NLS= prefix=/usr %{?_smp_mflags}
...
make install prefix=/usr DESTDIR=%{buildroot} ENABLE_NLS=
rm -f %{buildroot}%{_infodir}/dir
find %{buildroot} -name '*.la' -delete 2>/dev/null || true
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/dos2unix.files

%files -f %{_builddir}/dos2unix.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 7.5.2-1
