...
CFLAGS="-Os -D_GNU_SOURCE -std=gnu11 -I$SYS/usr/include -Wno-implicit-function-declaration -Wno-incompatible-pointer-types -Wno-int-conversion $UAPI" \
LDFLAGS="-Wl,-rpath,/usr/lib -Wl,-rpath-link,$SYS/usr/lib -L$SYS/usr/lib " \
PKG_CONFIG_PATH="$SYS/usr/lib/pkgconfig" \
./configure --build=x86_64-pc-linux-gnu --host=%{_target_cpu}-linux-musl --cache-file=config.cache --disable-shared --enable-static --enable-all-static --with-oniguruma=builtin --disable-maintainer-mode --disable-docs --prefix=/usr
make %{?_smp_mflags}
