...
CFLAGS="-Os -D_GNU_SOURCE -fPIC -I$SYS/usr/include -Wno-poison-system-directories -Wno-implicit-function-declaration -Wno-incompatible-pointer-types -Wno-int-conversion $UAPI" \
...
./configure --build=x86_64-pc-linux-gnu --host=$TRIPLE --prefix=/usr --with-shared --with-termlib --without-normal --without-debug --without-ada --without-cxx --without-cxx-binding --without-manpages --without-progs --without-tack --without-tests --enable-pc-files --with-pkg-config-libdir=/usr/lib/pkgconfig --disable-db-install --enable-widec --enable-overwrite --with-default-terminfo-dir=/usr/share/terminfo --with-terminfo-dirs=/etc/terminfo:/lib/terminfo:/usr/share/terminfo
make %{?_smp_mflags}
...
make install DESTDIR=%{buildroot} INSTALL='install -p' && (cd %{buildroot}/usr/lib && for l in tinfo ncurses form menu panel; do [ -e lib${l}w.so ] && ln -sf lib${l}w.so lib${l}.so; done; cd %{buildroot}/usr/lib/pkgconfig && for p in tinfo ncurses form menu panel; do [ -e ${p}w.pc ] && ln -sf ${p}w.pc ${p}.pc; done; true)
