...
Version:        4.4.36
Release:        1%{?dist}
Summary:        libxcrypt -devel (oxide)
License:        LGPL-2.1-or-later
Source0:        libxcrypt-4.4.36.tar.xz

%description
libxcrypt -devel (oxide)
...
./configure --build=x86_64-pc-linux-gnu --host=$TRIPLE --prefix=/usr --enable-shared --disable-static --disable-werror --enable-hashes=glibc --enable-obsolete-api=glibc
make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/libxcrypt.files

%files -f %{_builddir}/libxcrypt.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 4.4.36-1
