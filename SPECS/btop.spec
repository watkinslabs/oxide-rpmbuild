...
Version:        1.4.0
Release:        1%{?dist}
Summary:        btop monitor (oxide)
License:        Apache-2.0
Source0:        btop-1.4.0.tar.gz
...
make CXX="$CXX" STATIC=true GPU_SUPPORT=false %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/btop.files

%files -f %{_builddir}/btop.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 1.4.0-1
