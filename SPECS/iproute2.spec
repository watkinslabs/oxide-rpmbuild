...
Version:        6.10.0
Release:        1%{?dist}
Summary:        iproute2 — ip/tc/bridge (oxide)
License:        GPL-2.0-or-later
Source0:        iproute2-6.10.0.tar.gz

%description
iproute2 — ip/tc/bridge (oxide)
...
export CFLAGS="-Os -fPIC -D_GNU_SOURCE -Wno-implicit-function-declaration -Wno-incompatible-pointer-types -I$SYS/usr/include -Wno-poison-system-directories $UAPI"
...
./configure && make CC="$CC" HOSTCC=gcc %{?_smp_mflags}
...
make install DESTDIR=%{buildroot} HOSTCC=gcc
rm -f %{buildroot}%{_infodir}/dir
find %{buildroot} -name '*.la' -delete 2>/dev/null || true
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/iproute2.files

%files -f %{_builddir}/iproute2.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 6.10.0-1
