...
Version:        1.14.10
Release:        1%{?dist}
Summary:        D-Bus message bus (oxide)
License:        GPL-2.0-or-later OR AFL-2.1
Source0:        dbus-1.14.10.tar.gz

%description
D-Bus message bus (oxide)
...
./configure --build=x86_64-pc-linux-gnu --host=$TRIPLE --prefix=/usr --localstatedir=/var --sysconfdir=/etc --runstatedir=/run --disable-systemd --disable-selinux --disable-tests --disable-doxygen-docs --disable-xml-docs --disable-ducktype-docs --without-x
make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/dbus.files

%files -f %{_builddir}/dbus.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 1.14.10-1
