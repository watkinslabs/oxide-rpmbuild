...
Version:        1.3.1
Release:        1%{?dist}
Summary:        zlib compression -devel (oxide)
License:        Zlib
Source0:        zlib-1.3.1.tar.gz

%description
zlib compression -devel (oxide)
...
CC="$CC" ./configure --prefix=/usr && make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/zlib.files

%files -f %{_builddir}/zlib.files
%changelog
