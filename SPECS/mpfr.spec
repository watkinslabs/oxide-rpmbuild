...
Version:        4.2.1
Release:        1%{?dist}
Summary:        GNU MPFR float library (oxide)
License:        LGPL-3.0-or-later
Source0:        mpfr-4.2.1.tar.gz

%description
GNU MPFR float library (oxide)
...
./configure --build=x86_64-pc-linux-gnu --host=$TRIPLE --prefix=/usr --with-gmp=$SYS/usr
make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/mpfr.files

%files -f %{_builddir}/mpfr.files
%changelog
* Sat Jun 13 2026 Chris Watkins <chris@watkinslabs.com> - 4.2.1-1
