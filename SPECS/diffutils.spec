...
./configure --build=x86_64-pc-linux-gnu --host=$TRIPLE --disable-nls --without-selinux --prefix=/usr
make %{?_smp_mflags}
...
( cd %{buildroot} && find . -type f -o -type l ) | sed 's#^\.##' | LC_ALL=C sort > %{_builddir}/diffutils.files

%files -f %{_builddir}/diffutils.files
%changelog
